...
function cmpLn(cur: number, ref: number, refFmt: string): string {
  if (ref === 0) return '';
  const d = diffPct(cur, ref);
  if (Math.abs(d) <= 5) return `na sua média habitual (${refFmt}).`;
  return d > 0 ? `${d}% acima da sua média (${refFmt}).` : `${Math.abs(d)}% abaixo da sua média (${refFmt}).`;
}
...
function buildDay(
  ...
  return [
    {
      icon: Droplet, color: 'text-blue-400', label: 'Água',
      ...
      line: avgW > 0
        ? `Média dos últimos 30 dias: ${fmtW(Math.round(avgW))}/dia — ${cmpLn(todayLog.water_ml, avgW, fmtW(Math.round(avgW)))}`
    ...
    {
      icon: Footprints, color: 'text-emerald-400', label: 'Passos',
      ...
      line: avgSt > 0
        ? `Média dos últimos 30 dias: ${fmtN(avgSt)} passos — ${cmpLn(todayLog.steps, avgSt, fmtN(avgSt))}`
        : 'Ainda sem histórico.',
      sentiment: s(todayLog.steps >= goals.steps, todayLog.steps > 0 ? 'ok' : 'warn'),
    },
    {
      icon: Dumbbell, color: 'text-rose-400', label: 'Academia',
      value: todayLog.gym_done ? 'Fui hoje! ✓' : 'Ainda não fui',
      line: `Últimos 30 dias: ${gymN} treinos (${Math.round(gymN / n * 100)}% dos dias). ${
        todayLog.gym_done
          ? 'Sequência mantida!'
    ...
    {
      icon: Pill, color: 'text-purple-400', label: 'Creatina',
      value: todayLog.creatine_done ? 'Tomei hoje ✓' : 'Ainda não tomei',
      line: `Consistência últimos 30 dias: ${crN}/${n} dias (${Math.round(crN / n * 100)}%). ${
        !todayLog.creatine_done && crN / n >= 0.9 ? 'Sua consistência é ótima — não deixa passar!' : ''
      }`,
      sentiment: s(todayLog.creatine_done, crN / n >= 0.8 ? 'ok' : 'warn'),
    },
    {
      icon: BookOpen, color: 'text-amber-400', label: 'Leitura',
      ...
      line: avgPg > 0
        ? `Média: ${r1(avgPg)} págs/dia nos últimos 30 dias — ${cmpLn(todayLog.reading_pages, avgPg, `${r1(avgPg)} págs`)}`
        : 'Ainda sem histórico.',
      sentiment: s(todayLog.reading_pages >= goals.reading_pages, todayLog.reading_pages > 0 ? 'ok' : 'warn'),
    },
    {
      icon: GraduationCap, color: 'text-violet-400', label: 'Estudo',
      ...
      line: avgSy > 0
        ? `Média: ${fmtT(Math.round(avgSy))}/dia — ${cmpLn(todayLog.study_minutes, avgSy, fmtT(Math.round(avgSy)))}`
        : 'Ainda sem histórico.',
      sentiment: s(todayLog.study_minutes >= goals.study_minutes, todayLog.study_minutes > 0 ? 'ok' : 'warn'),
    },
    {
      icon: Wind, color: 'text-teal-400', label: 'Meditação',
      ...
      line: avgMd > 0
        ? `Média: ${fmtT(Math.round(avgMd))}/dia — ${cmpLn(todayLog.meditation_minutes, avgMd, fmtT(Math.round(avgMd)))}`
        : 'Ainda sem histórico.',
      sentiment: s(todayLog.meditation_minutes >= goals.meditation_minutes, todayLog.meditation_minutes > 0 ? 'ok' : 'warn'),
    },
    {
      icon: Film, color: 'text-violet-300', label: 'Filmes',
      ...
      line: (() => {
        ...
        return `Últimos 7 dias: ${n7} filme${n7 !== 1 ? 's' : ''}. Últimos 30 dias: ${n30} filmes.`;
      })(),
      sentiment: 'na',
    },
    {
      icon: Activity, color: 'text-cyan-400', label: 'Aeróbico',
      value: tdAer > 0 ? `${tdAer} sessão${tdAer > 1 ? 'ões' : ''} hoje` : 'Nenhuma sessão hoje',
      line: (() => {
        const parts: string[] = [];
        if (daysAgo !== null && daysAgo > 0) {
          parts.push(`Última sessão: há ${daysAgo} dia${daysAgo > 1 ? 's' : ''} — ${lastS!.distance_km.toFixed(1)} km a ${lastS!.avg_speed_kmh.toFixed(1)} km/h`);
        }
        const c4w = new Date(); c4w.setDate(c4w.getDate() - 28);
        const n4w = aerobic.filter(s => s.date >= dateStr(c4w)).length;
        if (n4w > 0) parts.push(`${n4w} sessões nas últimas 4 semanas (${r1(n4w / 4)}/sem em média)`);
        return parts.join('. ') || 'Nenhuma sessão recente registrada.';
      })(),
  ...
  return [
    {
      ...
      line: aW > 0 ? `Média em semanas anteriores: ${fmtW(Math.round(aW))} — ${cmpLn(wW, aW, fmtW(Math.round(aW)))}` : noHist,
    ...
    },
    {
      ...
      line: aS > 0 ? `Média: ${fmtN(aS)} passos — ${cmpLn(wS, aS, fmtN(aS))}` : noHist,
    ...
    },
    {
      ...
      line: aG > 0 ? `Média das últimas 4 semanas: ${r1(aG)} treinos — ${cmpLn(wG, aG, `${r1(aG)} treinos`)}` : `Meta semanal: ${goals.gym_days_per_week} dias.`,
      sentiment: wG >= goals.gym_days_per_week ? 'great' : wG > 0 ? 'ok' : 'warn',
    },
    {
      icon: Pill, color: 'text-purple-400', label: 'Creatina',
      value: `${wC}/${elapsed} dias ${pl}`,
      line: aC > 0 ? `Média: ${r1(aC)} dias — ${cmpLn(wC, aC, `${r1(aC)} dias`)}` : noHist,
    ...
    },
    {
      ...
      line: aP > 0 ? `Média: ${Math.round(aP)} págs — ${cmpLn(wP, aP, `${Math.round(aP)} págs`)}` : noHist,
    ...
    },
    {
      ...
      line: aY > 0 ? `Média: ${fmtT(Math.round(aY))} — ${cmpLn(wY, aY, fmtT(Math.round(aY)))}` : noHist,
    ...
    },
    {
      ...
      line: aM > 0 ? `Média: ${fmtT(Math.round(aM))} — ${cmpLn(wM, aM, fmtT(Math.round(aM)))}` : noHist,
    ...
    },
    {
      ...
      line: aMov > 0
        ? `Média: ${r1(aMov)} filmes/semana — ${cmpLn(wMov, aMov, `${r1(aMov)} filmes`)}`
        : wMov > 0 ? `${wMov} filme${wMov > 1 ? 's' : ''} esta semana.` : 'Nenhum filme esta semana.',
      sentiment: 'na',
    },
    {
      icon: Activity, color: 'text-cyan-400', label: 'Aeróbico',
      ...
      line: (() => {
        const parts: string[] = [];
        if (aAer > 0) parts.push(`Média: ${r1(aAer)} sessões/semana — ${cmpLn(wAer.length, aAer, `${r1(aAer)} sessões`)}`);
        if (wAer.length > 0) {
          const km  = wAer.reduce((s, x) => s + x.distance_km, 0);
          const spd = avg(wAer.map(x => x.avg_speed_kmh));
          parts.push(`${km.toFixed(1)} km, vel. média ${spd.toFixed(1)} km/h`);
        }
...
function buildMonth(
  ...
  const noHist = 'Sem histórico suficiente.';
  const cr = 'Média no mesmo período dos últimos 3 meses';
  ...
  return [
    {
      ...
      line: aW > 0 ? `${cr}: ${fmtW(Math.round(aW))} — ${cmpLn(mW, aW, fmtW(Math.round(aW)))}` : noHist,
    ...
    },
    {
      ...
      line: aS > 0 ? `${cr}: ${fmtN(aS)} — ${cmpLn(mS, aS, fmtN(aS))}` : noHist,
    ...
    },
    {
      ...
      line: aG > 0 ? `${cr}: ${r1(aG)} treinos — ${cmpLn(mG, aG, `${r1(aG)} treinos`)}` : `Meta de ${gymTarget} treinos este mês.`,
    ...
    },
    {
      ...
      pct: pct(mC, dom),
      line: aC > 0 ? `${cr}: ${r1(aC)} dias — ${cmpLn(mC, aC, `${r1(aC)} dias`)}` : noHist,
    ...
    },
    {
      ...
      line: aP > 0 ? `${cr}: ${Math.round(aP)} págs — ${cmpLn(mP, aP, `${Math.round(aP)} págs`)}` : noHist,
    ...
    },
    {
      ...
      line: aY > 0 ? `${cr}: ${fmtT(Math.round(aY))} — ${cmpLn(mY, aY, fmtT(Math.round(aY)))}` : noHist,
    ...
    },
    {
      ...
      line: aM > 0 ? `${cr}: ${fmtT(Math.round(aM))} — ${cmpLn(mM, aM, fmtT(Math.round(aM)))}` : noHist,
    ...
    },
    {
      ...
      line: aMov > 0
        ? `${cr}: ${r1(aMov)} filmes — ${cmpLn(mMov, aMov, `${r1(aMov)} filmes`)}`
        : `${mMov} filmes este mês.`,
      sentiment: 'na',
    },
    {
      icon: Activity, color: 'text-cyan-400', label: 'Aeróbico',
      ...
      line: (() => {
        const parts: string[] = [];
        if (aAer > 0) parts.push(`${cr}: ${r1(aAer)} sessões — ${cmpLn(mAer.length, aAer, `${r1(aAer)} sessões`)}`);
        if (mAer.length > 0) {
          const km  = mAer.reduce((s, x) => s + x.distance_km, 0);
          const spd = avg(mAer.map(x => x.avg_speed_kmh));
          parts.push(`${km.toFixed(1)} km no total, vel. média ${spd.toFixed(1)} km/h`);
        }
...
function buildYear(
  ...
  const lyCmp = (cur: number, prev: number, fmtFn: (n: number) => string): string => {
    if (!hasLY) return `Sem dados de ${y - 1} para comparar.`;
    if (prev === 0) return 'Sem dados do ano anterior.';
    const d = diffPct(cur, prev);
    return `Mesmo período em ${y - 1}: ${fmtFn(prev)} — ${
      Math.abs(d) <= 5 ? 'na média habitual' : d > 0 ? `+${d}% a mais` : `${Math.abs(d)}% a menos`}.`;
  };
  ...
  return [
    ...
    },
    {
      ...
      line: lyCmp(cyG, lyG, n => `${n} treinos`),
    ...
    },
    {
      ...
      line: (() => {
        ...
        if (hasLY && lyAer > 0) {
          ...
          parts.push(`Mesmo período em ${y - 1}: ${lyAer} sessões — ${
            Math.abs(d) <= 5 ? 'na média' : d > 0 ? `+${d}% a mais` : `${Math.abs(d)}% a menos`}`);
        }
        if (cyAer.length > 0) {
          const km  = cyAer.reduce((s, x) => s + x.distance_km, 0);
          const spd = avg(cyAer.map(x => x.avg_speed_kmh));
          parts.push(`${km.toFixed(0)} km no total, vel. média ${spd.toFixed(1)} km/h`);
        }
...
const SENT_BORDER: Record<Sentiment, string> = {
  ...
  ok:    'border-amber-500/10',
  warn:  'border-rose-500/15',
  na:    'border-white/10',
};

const SENT_VALUE: Record<Sentiment, string> = {
...
function InsightCard({ ins }: { ins: Insight }) {
  const { icon: Icon, color, label, value, pct: p, line, sentiment } = ins;
  return (
    <div className={`rounded-[2rem] border ${SENT_BORDER[sentiment]} bg-white/5 p-5 backdrop-blur-xl`}>
      <div className="mb-2.5 flex items-center gap-2">
        <Icon size={14} className={color} />
        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{label}</span>
      </div>
      <p className={`mb-2 text-base font-bold leading-snug ${SENT_VALUE[sentiment]}`}>{value}</p>
      {p !== undefined && (
        <div className="mb-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-800/60">
          <div
      ...
      <p className="text-sm leading-relaxed text-slate-400">{line}</p>
    </div>
  );
}
